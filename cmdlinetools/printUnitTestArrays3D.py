...
#  file = "test_sml_3D.hdf5"
file = "test_sml_multiscale_3D.hdf5"
h_tolerance = 1.0e-6
...
if file == "test_sml_multiscale_3D.hdf5":
    dx_inner = 0.0175
    dx_outer = 0.025
else:
    dx_inner = 0.1
...
inner_mask = np.logical_and(inner_mask, coords[:, 1] < (0.5 * boxsize + dx_inner))
inner_mask = np.logical_and(inner_mask, coords[:, 2] > (0.5 * boxsize - dx_inner))
inner_mask = np.logical_and(inner_mask, coords[:, 2] < (0.5 * boxsize + dx_inner))
...
outer_mask = np.logical_and(outer_mask, coords[:, 1] < 0.5 * boxsize + dx_outer)
outer_mask = np.logical_and(outer_mask, coords[:, 2] > 0.5 * boxsize - dx_outer)
outer_mask = np.logical_and(outer_mask, coords[:, 2] < 0.5 * boxsize + dx_outer)
